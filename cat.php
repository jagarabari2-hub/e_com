<?php
  ...
  else  
  {
	...
	$con=mysqli_connect('localhost','root','','e_com') or die("db not");
	if(isset($_POST['b1']))
	{
		//	echo $_POST['t2'];
				$q="insert into category(name) values('".$_POST['t2']."')";
	
		$rs=mysqli_query($con,$q) or die("query not");
	}
 ?>
<html>
<head>
...
<link rel="stylesheet" href="ocss/cat.css">
...
<h2 class="txt-1">Category </h2><hr>
      

           
            
            
            <h4 class="txt-2">Enter Category Name : </h4><input type="text" name="t2" class="inp-1" required>
           
<input type="submit" class="inp-2" name="b1" value="add category">

        </form><br><br>
		<hr>
		<h2 class="txt-3">View Category</h2>
		<hr>
		<br><br>
		...
		$q="select * from category LIMIT $start_from, $limit";
		...
		if(mysqli_num_rows($rs)>0)
		{
			foreach($rs as $h)
			{
				...
				<div class="col"><a class=sour href="edit_cat.php?sid=<?= $h['id']; ?>">Edit</a></div>
				<div class="col"><a class=sour href="del_cat.php?sid=<?= $h['id']; ?>">Delete</a></div>
				</div><hr>
				<?php
			}
		}
		...
		<?php
				...
				for ($i=1; $i<=$total_pages; $i++) {
					if ($i==$pn)
					{
						$pagLink .= "<li class='active-1'><a class=j1 href='cat.php?page="
														  .$i."'>".$i."</a></li>";
					}
					else
					{
						$pagLink .= "<li class='active-1'><a class=j1 href='cat.php?page=".$i."'>
														  ".$i."</a></li>";
					}
				};
				echo $pagLink;
